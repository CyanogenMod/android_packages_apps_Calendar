    ...
    protected void onResume() {
        ...
        new AsyncQueryHandler(getContentResolver()) {
            ...
            protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
                ...
                if (null != mIcicle) {
                    long[] checkChangesAccountIds = mIcicle
                            .getLongArray(CHECK_CHANGES_ACCOUNT_ID_ARRAY);
                    boolean[] checkChanges = mIcicle.getBooleanArray(CHECK_CHANGEDS_ARRAY);
                    if (null != checkChangesAccountIds && null != checkChanges) {
                        for (int i = 0; i < checkChangesAccountIds.length; i++) {
                            mAdapter.getCalendarChanges().put(checkChangesAccountIds[i],
                                    checkChanges[i]);
                            mAdapter.getCalendarInitialStates().put(checkChangesAccountIds[i],
                                    checkChanges[i]);
                        }
                    }
                }
